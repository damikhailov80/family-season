...
| `/api/library` | Избранное, свой сохранённый сезон и публикация — для живого постера | Роут-хендлер |
...
| Чтение для живого постера (один запрос на все ответы) | `src/app/api/library/route.ts`, `src/state/useLibrary.ts` |
...
## Витрина: публикация, лайки и жалобы

Третья, четвёртая и пятая таблицы. Сохранённый сезон можно **выложить на витрину**
«Идеи сообщества»; чужой выложенный — лайкнуть и пожаловаться на него с комментарием.

| Что | Где |
| --- | --- |
| Пределы, порог жалоб, `CommunityStatus`, комментарий | `src/model/community.ts` |
| Публикация, поиск по адресу, лайк, жалоба | `src/server/community.ts` |
| Действия постера | `src/server/actions.ts` |
| Ответ живому постеру | `src/app/api/library/route.ts`, `src/state/useLibrary.ts` |
| Кнопки мегафона, сердца и флажка | `src/components/edit/Toolbar.tsx` |
| Окно жалобы | `src/components/edit/ReportDialog.tsx` |
| Витрина и мини-постер | `src/app/ideas/page.tsx`, `src/components/community/SeasonPreview.tsx` |
| Жалоба прямо с витрины (клиентская) | `src/app/ideas/ReportEntry.tsx` |
| Пометка и снятие в кабинете (клиентское) | `src/app/seasons/page.tsx`, `UnpublishEntry.tsx` |
| Схема | `tools/db/schema.sql` (`shared_seasons`, `shared_likes`, `shared_reports`) |

- **Публикуется не адрес, а строка `seasons`.** `shared_seasons` — это `season_id` плюс
  владелец, и ни `url`, ни `title` в ней нет: и то и другое берётся join'ом из самого
  сезона. Отсюда всё остальное поведение: переименовали сезон в кабинете — поменялось и на
  витрине, сохранили новую версию — витрина показывает её, удалили сезон — публикация ушла
  каскадом. Колонки «адрес» и «название» рядом были бы **той самой второй копией**, ради
  запрета которой писан раздел «Ссылка». Отсюда же и условие кнопки: **выложить можно
  только сохранённый сезон** — публиковать нечего, пока нечего указывать.
- **Первые внешние ключи в проекте.** Лайк и жалоба без публикации бессмысленны,
  публикация без сезона — тем более, и чистить это руками пришлось бы в трёх местах.
  `on delete cascade` — не оптимизация, а само правило.
- **Счётчика лайков нет, есть ряды.** Число выводится через `count(*)`, как месяц и тема
  в списке выводятся из адреса, и по той же причине. Первичный ключ `(shared_id,
  account_key)` — это и есть правило «один человек — один лайк»; предела на лайки нет,
  их просто считают.
- **Желаемое состояние лайка приходит от клиента** (`setLike(id, on)`), а не вычисляется
  на сервере. Не лень: «переключить» одним оператором не выходит — удаление и вставка
  в общем CTE не видят работы друг друга и дерутся за первичный ключ. Так запрос один
  и **идемпотентен**, а повторное нажатие в соседней вкладке ничего не ломает.
- **Повторная жалоба уточняет прежнюю** (`on conflict … do update`), а не заводит вторую.
  Поэтому порог «больше пяти» считает **авторов**, а не нажатия, — иначе спрятать чужой
  сезон мог бы один человек. Предел в сто жалоб на аккаунт при этом считается по чужим
  публикациям (`shared_id <> $1`): правка своей же жалобы не имеет права в него упереться.
- **Своё не лайкают и на своё не жалуются.** Кнопок у своего сезона нет вовсе, но проверка
  стоит и на сервере — кнопки удобство, а не рубеж защиты. Её ответ — отдельный статус
  `own`, разведённый с `error` по той же причине, что и `limit`: человеку понятно, что
  случилось, и чинить нечего.
- **`findShared` работает без входа, и это обязательно.** «Выложен ли этот сезон» — факт
  публичный, и в `/api/library` это единственный ответ **не про спрашивающего**. Иначе
  анониму нечего было бы нажать, а предложение войти он должен получать в ответ на
  нажатие — ровно как со звёздочкой (`anonymous` → `LoginDialog`).
- **У витрины свой набор слов** (`COMMUNITY_TEXT` в тулбаре): «не удалось сохранить» про
  лайк — неправда. Показывается всё тем же тостом, механизм по-прежнему один.
- **Один и тот же адрес мог выложить не один человек** — `findShared` берёт того, кто
  раньше (`order by created_at asc limit 1`). Вторая публикация того же постера безвредна:
  своя строка у каждого своя, и убирает её каждый свою.
- **`/privacy` описывает содержимое базы буквально.** Появилась витрина — страница
  переписана тем же изменением, включая главное: **выложенный сезон виден всем**, а всё
  напечатанное на постере лежит в его адресе.
...
- **Превью — мини-постер, а не карточка сайта,** и потому носит тему: `data-palette`
  висит на самом превью, как кружок в «Моих сезонах». Это единственное исключение из
  правила «карточки на сайте тему не носят» — оно про карточки, а тут изображение листа.
  Дальше превью атрибут не уходит, сайт вокруг своих цветов не меняет.
- **На превью нет личных проектов, настроений и итогов.** Это и есть то, ради чего сезон
  открывают: витрина заманивает, а не заменяет постер. Показываются шапка, месяц, тема
  с вопросом, четыре недели и цель — и всё из **разобранного адреса**, второго источника
  у превью нет.
- **С витрины можно пожаловаться, но не лайкнуть.** Лайкают, посмотрев сезон; брань,
  наоборот, видно сразу — за тем флажок здесь и стоит. `ReportEntry` — третий клиентский
  компонент сайта, причина та же, что у `DeleteEntry`: без JS окно с комментарием стоило
  бы отдельного экрана. Окна и слова у него общие с постером (`ReportDialog`,
  `LoginDialog`, `COMMUNITY_TEXT` из модели) — вторых копий не заводим.
- **Не прочитали витрину — пустота и тост**, никогда не умолчание: то же правило, что
  у кабинета.

В «Моих сезонах»:

- **Список своих сезонов идёт левым join'ом на `shared_seasons`,** у избранного этого
  join'а нет вовсе: публикация бывает только у своего сезона, а подставлять чужой вкладке
  колонки-константы значило бы врать в самом запросе. `Entry.sharedId` и `Entry.likes`
  у избранного поэтому просто `null` и `0`.
- **«На витрине» — пометка в строке метаданных, а не кнопка.** Это состояние строки, и
  рамка вокруг сделала бы из него действие. Лайки показываются, только когда они есть.
- **Выложить из кабинета нельзя, снять — можно.** Делятся тем, что видят на постере, и
  кнопка публикации живёт там; а вот убрать выложенное человек должен уметь оттуда, где
  видит список. `unpublishEntry` — отдельное действие от `togglePublish`, потому что
...
- **Снятие подтверждается окном:** вместе с публикацией каскадом уходят её лайки, и
  выложив сезон заново, человек начнёт с нуля. Сказать это можно только заранее.
...
| Поделиться с сообществом, Скопировать ссылку, Печать / PDF (иконки, группой у правого края) | да | нет | да, ссылка вместе с `data=` |
| В избранное (звёздочка, первой в ряду) | да | да | да, вместе с `data=` |
| Лайк и жалоба (сердце и флажок, за звёздочкой) | да, если сезон выложен кем-то другим | то же | нет: пример не выложен |
...
- **кнопки без подписи стоят по краям, и это два разных края.** Слева, до подсказки, —
  то, что делают с этим постером у себя: звёздочка, а за ней лайк и жалоба. Справа, за
  чертой, — то, что уносит постер наружу: «Поделиться с сообществом», ссылка и печать.
  Место у звёздочки постоянное именно потому, что она видна всегда: появляйся она через
  раз, подсказка рядом прыгала бы при каждом переходе. Правая группа отделена чертой,
  а не пустотой: пустоту в переносящемся ряду не отличить от переноса; ширину под неё
  держит `padding-right` у `.bar`, и **при добавлении кнопки его надо увеличить** —
  группа вынута из потока и сама места себе не просит. Подписи всем заменяют `title`
  и `aria-label` — рядом с текстовыми кнопками короткие надписи только утяжеляли бы ряд;
- **состояние переключателя показывает заливка кнопки, а не рисунок.**
  `.icon[aria-pressed='true']` в `Toolbar.module.css` — единственный признак «включено»
  у всех четырёх переключателей (звёздочка, лайк, жалоба, «Поделиться»). Через `filled`
  договориться нельзя: у звезды и сердца залитая форма есть, у мегафона её нет.
  Правило обязано стоять **после** `.icon:hover` — специфичность у них одинаковая;
- **«Поделиться» рисуется всегда, а не появляется**, и погашена, пока сезон не сохранён:
  подсказка объясняет почему. Исчезающая кнопка в вынутой из потока группе меняла бы её
  ширину, то есть двигала бы весь верхний ряд;
- **лайк и жалобы нет у своего сезона и у невыложенного** — там их некому и не за что
  показывать. Это единственные кнопки тулбара, чья видимость зависит от ответа сервера;
