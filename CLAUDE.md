...
- Вход (`site/LoginButtons.tsx`) настоящий и **целиком серверный** — см. раздел «Вход».
  Клиентских компонентов в обвязке два, и оба заведены сознательно: `Toast` и
  `NewSeasonButton` (кнопка «Новый сезон» — ей нужно окно с именем).
...
- **`Toast` — один из двух клиентских компонентов в самой обвязке сайта** (шапка, подвал;
  второй — `NewSeasonButton`). Иначе никак: сообщение
...
- **Вход из окна «черновик будет затёрт» доводит дело до конца.** `GoogleLoginButton`
  получает `returnTo` = `/seasons?claim=1`, и `ClaimDraft` там забирает черновик строкой,
  чистит `localStorage` и снимает пометку через `replaceState` — иначе перезагрузка
  завела бы вторую строку. Забираем **только по пометке**: заход вошедшего в кабинет — не
  согласие отдать в коллекцию то, что лежало в браузере. Неудача черновик не трогает.
- **Вошедшему черновик на `/seasons` не показываем.** У него коллекция в базе, а
  единственный момент, когда залежавшийся черновик важен, закрывает `?claim=1`.
...
- **Скрытый сезон — не удалённый.** Он открывается по своему адресу, принимает лайки и
  форки, но в «Идеях» его нет. Панель об этом честно говорит подсказкой.
- **Название публикации не даётся руками** — оно выводится из содержимого
  (`defaultSeasonTitle`). Колонки рядом нет: это была бы вторая копия того, что уже лежит
  в `content`.
...
- **На превью нет личных проектов, настроений и итогов.** Это и есть то, ради чего сезон
  ...
  второго источника у превью нет.
...
| --- | --- | --- |
| Черновик `/sheet*` | `app/sheet/DraftBar.tsx` | Править/Готово, печать и одно действие по входу: вошедшему «Сохранить в мои сезоны», остальным «Войти и сохранить» |
...
- **панель черновика знает про вход заранее** (проп со страницы) и потому не обещает того,
  ...
  нет, и прежняя панель вела его к ней через два окна подряд — имя, а следом «нужен
  вход», — чтобы упереться в стену. Теперь у него одна кнопка «Войти и сохранить»
  (`loginWithGoogle` с возвратом на `?claim=1`), у вошедшего — «Сохранить в мои сезоны» в
  одно нажатие. Это **единственное** место, где вход спрашивается заранее ради подписи на
  кнопке, а не ради того, куда ляжет сезон;
...
- **два разговора о потере, и путать их нельзя.** Невошедшему окно **всегда** говорит, что
  сезон живёт только в этом браузере, и **всегда** показывает вход (`DRAFT_ONLY_HERE`,
  `offerLogin`): первый сезон теряется от чистки данных сайта ровно так же, как десятый,
  просто его хозяин узнаёт об этом позже. А красное предупреждение (`warning`,
  `draftWillBeLost`) добавляется, только когда черновик уже есть: оно про **сейчас**, и
  потому называет имя — «черновик будет затёрт» человек проверить не может, а
  «„Сентябрь у бабушки“ будет затёрт» может. Слова обоих лежат в `draft.ts`: окна
  заведения два, и вторая копия текста разошлась бы с первой;
...
- **про вход панель ничего не знает и не спрашивает.** Действие уходит на сервер, и если
