...
- **подсказка называет место и сезон, а не объясняет устройство.** В левом углу ряда
  ...
  браузере», «Сезон по ссылке»), а свой сезон в правке — вовсе не место, а слово о записи
  («Сохраняется само»): она там важнее. **Название берётся из содержимого** (`ideaTitle`),
  а не из колонки `title` и не пропом со страницы: панель стоит внутри провайдера, и
  название обязано меняться вместе с темой месяца, которую тут же и правят. У снятого с
  витрины места нет, и остаётся одно название; сам `<span>` при этом не исчезает никогда —
  он разводит кнопки по краям ряда.
