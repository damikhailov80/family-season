...
- **Первый `<dialog>` в проекте — и образец для остальных** (окна сохранения, входа и
  ...
  **Прокрутка под открытым окном заперта одним правилом на весь сайт**
  (`html:has(dialog:modal)` в `global.css`): своего запирания окну заводить не надо,
  а `scrollbar-gutter: stable` там же держит ширину страницы, чтобы вёрстка не
  дёргалась на открытии. Замена затирает имена, а при
