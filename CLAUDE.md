...
| `/sheet` | Лист-пример | `'use client'` + `dynamic(ssr: false)` |
| `/sheet#d=…` | Свой лист | то же |
| `/sheet#new=1` | Пустой бланк сразу в правке | то же |
...
- `src/app/sheet/page.tsx` грузит лист через `next/dynamic` с **`ssr: false`**. Так надо: демо
...
- Кроме `d=` в хэше бывают две пометки, и обе — не часть шаблона: в присланную ссылку
  (`buildShareUrl`) они не попадают и стираются первой же синхронизацией URL.
  - `edit=1` — «открыть сразу в правке», ставят ссылки кнопок форка;
  - `new=1` — «сделай пустой бланк», приходит с лендинга: закодировать шаблон на сервере
    он не может, поэтому просит лист сделать это самому. Разбирается и в `initialBoot`
    (`src/app/sheet/Sheet.tsx`), и в обработчике `hashchange` (`DocProvider`) — иначе
    смена одного хэша на уже открытом листе показала бы демо.
...
| --- | --- | --- |
| Пример | `/sheet` | `null` |
| Свой лист | `/sheet#d=…` | `{ own: true }` |

Лендинг (`/`) — отдельная запись перед ними обеими: с него в лист ведут обычные `<a href>`,
то есть полная навигация документа. Приход по `#new=1` записи в историю не добавляет —
позади лежит лендинг, а не пример, поэтому «К примеру» делает `pushState` на `/sheet`.

- `pushState` для листа — **только** в `startCustom`, а его зовёт единственная кнопка —
  «Форкнуть пример». Пустой сезон делается не кнопкой на листе, а ссылкой `#new=1`
  из шапки сайта, и записи в историю не добавляет. Правка
