...
| Переключатели | `components/site/LangSwitcher.tsx` (подвал), `app/[lang]/account/LanguageEditor.tsx` |
...
Разводить их обязательно: чужой выложенный сезон открывают из любого языка, и
`/ru/s/<code>` на польскую идею показывает русскую обвязку и польский лист. Экранные
...
Заодно `proxy` дописывает в запрос два заголовка: `x-lang-path` (путь без языка — иначе
лейауту некуда уводить, своего адреса серверный компонент не знает) и `x-lang-source`:
...
- **Витрина показывает свой язык** (`randomIdeas(lang)`): идею берут, чтобы прочитать, и
  десяток непонятных постеров ей ни к чему. Прямая ссылка при этом языком **не**
  фильтруется — пришедшему по ней сезон уже показали, и прятать его поздно.
