...
- **Сайт обязан работать при мёртвой базе.** Лендинг, примеры, постер, печать и сам вход
  от неё не зависят. Поэтому `query` в `db.ts` не бросает, а возвращает `null`, шапка
  откатывает ссылку, а кабинет честно говорит «настройки недоступны». Не заводите запросов,
  которые роняют страницу.
...
- **Состав уезжает аргументом действия, а не полями формы.** Это не вкусовщина: форму,
  ...
  `useActionState` не нужен: результат сообщается пометкой в адресе
  (`?ok=1` / `?offline=1` / `?stale=1`), и она переживает перезагрузку.
