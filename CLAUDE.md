...
| `/seasons` | «Мои сезоны» — список сохранённого, только для вошедших | Серверный компонент |
...
- **База уже есть, но лист в ней не лежит.** В PostgreSQL сегодня одна таблица —
  настройки аккаунта (см. «Настройки и база»). Инвариант ниже она не отменяет: постер
  в неё не попадает.
...
- **Решение принято, исполнено наполовину.** Владелец проекта согласился отменить этот
  инвариант: у листа появится серверная копия в PostgreSQL, кнопка «Сохранить» и список
  сохранённого в «Моих сезонах». Вход (Auth.js) уже сделан — см. «Вход»; **хранилища
  ещё нет**, `pg` не установлен, и раздел «Ссылка» ниже описывает сегодняшнюю правду и
  действует как есть. Переписывать его нужно в том же изменении, где появится сервер, —
  вход этого инварианта не касался и намеренно был отделён от него.
...
- **Хранилища нет, и это не пробел.** Адаптер БД не подключён, поэтому Auth.js держит
  сессию в зашифрованной куке (стратегия JWT), а на сервере не остаётся ни строчки о
  пользователе — имя и почта живут только в куке самого читателя. Об этом написано в
  двух местах — на `/seasons` и на `/privacy`; тексты уже пережили одну такую правку,
  когда появилась таблица настроек, и **правятся тем же изменением**, что и хранилище,
  иначе сайт начнёт врать о себе. `/privacy` при этом не
  украшение: без её адреса Google не выпускает вход из режима Testing в продакшен
...
- **Postgres входу не нужен был и не нужен.** База нужна кнопке «Сохранить», а не логину;
  вход поэтому сделан отдельным изменением. Когда хранилище появится, адаптер добавится
  **рядом**: Auth.js держит JWT и вместе с адаптером, стратегию менять не придётся.
...
Первая таблица в PostgreSQL. В ней лежит одна настройка — состав семьи для новых постеров.
...
| Страница ошибки кабинета | `src/app/account/error.tsx` |
...
- **Таблица настроек не отменяет инвариант «Ссылка».** Запрет из раздела выше — про
  **лист**: сервер не должен становиться его вторым хранилищем. В `user_settings` постера
  нет, там от двух до пяти пар «лицо — имя». Появится кнопка
  «Сохранить» и серверная копия постера — вот тогда раздел «Ссылка» и переписывается.
...
- **Первый и пока единственный `<dialog>` в проекте.** Замена затирает имена, а при
...
- **Сайт обязан работать при мёртвой базе — но кабинет обязан падать.** Граница проходит
  ровно здесь, и стирать её нельзя. Лендинг, примеры, постер, печать и вход от базы не
  зависят: `query` не бросает, `readFamily` отдаёт `null`, шапка молча откатывает ссылку
  «Новый сезон». А настройки без хранилища не существуют: `familyState` и `writeFamily`
  **бросают**, `/account` отдаёт 500 и показывает `src/app/account/error.tsx`. Мягкая
  плашка «попробуйте попозже» там была и убрана намеренно: она врала (умолчание вместо
  настоящих настроек), прятала аварию от владельца и превращала потерянное сохранение в
  незаметное. Не заводите запросов, которые роняют лендинг, — и не глушите те, что роняют
  кабинет.
- **Error boundary стоит на сегменте кабинета, а не в корне.** `src/app/account/error.tsx` —
  первый и пока единственный в проекте. В корне он накрыл бы весь сайт, то есть отменил бы
  границу из предыдущего пункта. Показывает `digest` ошибки: по нему её строка находится в
  логах сервера, и это единственный мостик от жалобы человека к причине.
- **«Не бросает» не значит «молчит».** `query` возвращает не `null`, а размеченный
  ...
  выключен `strict`, и булев союз TypeScript не сужает. Превращает результат в исключение
  один `rowsOrThrow` в `settings.ts` — там же, где проходит граница «сайт живёт, кабинет
  падает».
- **Лог — единственный след аварии, и он обязан отвечать «что чинить».** Поэтому в `catch`
  ...
  (`settings:read` — мягкое чтение шапки, `settings:read:account` — жёсткое чтение
  кабинета, `settings:write`). Сам SQL в лог **не пишем**: вместе с ним туда уехали бы
...
- **Состав уезжает аргументом действия, а не полями формы.** Это не вкусовщина: форму,
  ...
  Исходов у сохранения три, и все три расходятся намеренно. **Успех** уезжает пометкой
  в адрес (`?ok=1`): ему надо пережить перезагрузку. **Отказ базы** бросает и уводит на
  `error.tsx`: молча потерянное сохранение хуже страницы ошибки. **Причины про сессию**
  (`stale`, `anonymous`) возвращаются значением и показываются внутри формы — они чинятся
  самим человеком, и редирект тут только стёр бы набранный состав. Отсюда `useActionState`
  в редакторе. `?ok=1` он же и вычищает `replaceState`-ом после первого рендера — иначе
  перезагрузка показывала бы «Сохранено ✓» и через час.
- **Редактор ключуется по составу с сервера** (`key={JSON.stringify(family)}`): после
  ...
  состояние.
- **Не прочитали состав — не показываем редактор.** До него доходят только `ok` и `stale`:
  отказ базы сюда не попадает вовсе. Иначе редактор показал бы умолчание вместо настоящих
  настроек, а «Сохранить» — это `upsert`: он затёр бы то, чего мы не видели.
...
- Полноценных миграций нет намеренно: таблица одна, схема идемпотентна. Появится вторая-
  третья — тогда и нумерованные шаги, а пока это честный один шаг, а не заготовка.
