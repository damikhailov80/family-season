...
| `/seasons` | «Мои сезоны» — кабинет, только для вошедших | Серверный компонент |
...
- **Хранилища нет, и это не пробел.** Адаптер БД не подключён, поэтому Auth.js держит
  ...
  двух местах — на `/seasons` и на `/privacy`; появится Postgres, и **оба текста надо
  переписать тем же изменением**, иначе сайт начнёт врать о себе. `/privacy` при этом не
...
- **Набор данных — умолчание провайдера** (`openid email profile`), `authorization.params`
  не переопределяем: раз ничего не хранится, ужимать scope не за чем.
