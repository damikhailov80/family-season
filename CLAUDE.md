...
| Подтверждение удаления (клиентское) | `src/app/seasons/DeleteEntry.tsx` |
...
- **Удаление подтверждается окном**, и `DeleteEntry` — **второй клиентский компонент сайта**
  ...
  Сам список при этом остаётся серверным — клиентская здесь одна кнопка. Спрашиваем не для
  порядка: другой копии сезона у сайта нет, и если ссылка нигде не осталась, постер пропал.
