...
- **Вход — `<form>` с серверным действием, а не `onClick`.** Отсюда главное следствие:
  в браузер не уезжает ни строчки Auth.js, `SessionProvider` не нужен, мигания состояния
  нет, и вход работает без JS. Не переводите кнопки на `useSession` — это разом отменит
  всё перечисленное.
