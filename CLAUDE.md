...
- `src/app/sheet/page.tsx` и `src/app/sheet/edit/page.tsx` грузят черновик через
  `next/dynamic` с **`ssr: false`**: он лежит в браузере, серверу его негде взять, а
  `location` и хранилище доступны уже в первом рендере. Страницы `/season` и `/s`,
  наоборот, серверные — содержимое едет пропсом, и постер рендерится на сервере.
...
- **До постера настройка доезжает действием, а не ссылкой.** «Новый сезон» в шапке у
  вошедшего — серверное действие `createSeason`: оно собирает бланк по составу
  (`templateForFamily`), заводит строку и уводит человека в неё. Раньше бланк кодировался
  в `href` кнопки — теперь бланку место в базе. Постеру про базу знать по-прежнему нечего.
- **Невошедший получает черновик.** У него нет ни строки, ни настроек: кнопка ведёт на
  голый `/sheet/edit`, то есть на пустой бланк в браузере.
...
| Что | Где |
| --- | --- |
| Пределы, порог жалоб, `PublishStatus`, слова отказов | `src/model/community.ts` |
...
- **Снятие с витрины зависит от чужих людей.** Отложил кто-то сезон в избранное — строка
  остаётся и лишь помечается `hidden_at`: забирать отложенное у людей нельзя, а прямая
  ссылка уже разошлась. Не отложил никто — удаляем совсем. Лайки и форки удалению не
  мешают: лайк — знак внимания, форк — копия, которая и так живёт своей жизнью.
...
- **Повторная жалоба уточняет прежнюю** (`on conflict … do update`), а не заводит вторую.
  Поэтому порог `REPORTS_TO_HIDE` считает **авторов**, а не нажатия: иначе спрятать чужой
  сезон мог бы один человек. Предел в сто жалоб на аккаунт считается по чужим публикациям
  (`public_id is distinct from`): правка своей же жалобы не имеет права в него упереться.
- **Жалоба живёт дольше публикации.** В строке лежат `code` и `author_key` — снимок того,
  на кого жаловались: разбираются с человеком, а не со строкой. При удалении публикации
  `public_id` обнуляется, а жалоба остаётся.
...
| --- | --- | --- |
| Черновик `/sheet*` | `app/sheet/DraftBar.tsx` | Править/Готово, «Сохранить в мои сезоны», печать |
...
- **имя спрашивается один раз, окном** (`NewSeasonDialog`): список без имён нечитаем, а
  заводить строку молча слишком похоже на промах по кнопке. Окно одно на все случаи
  заведения сезона, и имени оно не спрашивает там, где сезону негде лежать: у невошедшего
  форк ложится черновиком, а у черновика имени не бывает;
