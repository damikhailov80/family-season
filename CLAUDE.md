...
- **Решение уже принято, но ещё не исполнено.** Владелец проекта согласился отменить этот
  ...
  сохранённого в «Моих сезонах»; под него берутся драйвер БД (`pg`) и библиотека
  авторизации (Auth.js). Пока хранилища нет, раздел «Ссылка» ниже описывает сегодняшнюю правду и действует
