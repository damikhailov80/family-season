/*
 * Сборка тем постера: source.json -> src/styles/palettes.css + src/model/palettes.data.ts.
 *
 *   node tools/palettes/build.mjs      (или npm run palettes)
 *
 * В source.json лежат сто наборов Canva как есть — четыре краски на набор.
 * Сами по себе они для листа не годятся: в наборе может оказаться и белый,
 * и почти чёрный, а плашке нужен читаемый текст, рамке — видимость на белой
 * бумаге. Поэтому здесь считается ровно то, чего CSS не умеет:
 *
 *   c1..c4      — краски набора, отсортированные от тёмной к светлой;
 *   on-c1..4    — цвет текста на плашке: белый или чернила (по контрасту);
 *   d1..d4      — тёмный оттенок каждой краски для рамок, заголовков и чернил.
 *
 * Всё остальное (чернила, линии сетки, подложки карточек, скотч полароида)
 * выводится из этих значений прямо в CSS — рецепт живёт в `src/styles/tokens.css`
 * и читается там целиком.
 *
 * Правила подбора:
 *   — сортировка по светлоте даёт постоянную лестницу «тёмная → светлая»,
 *     поэтому «глубокий» акцент (заголовок, рамка листа) всегда самый тёмный;
 *   — краска светлее L 0.95 подтемняется: белая плашка на белой бумаге исчезает;
 *   — тёмный оттенок — та же краска при L <= 0.45; у совсем светлых красок
 *     цветность заодно подтягивается, иначе тёмный оттенок выходит серым.
 */
