...
/**
 * Секции просят слот, набор подставляет в него рисунок — поэтому переключение
 * набора не трогает ни вёрстку, ни сами секции.
 *
 * Из `doodles/index.ts` не экспортируется: там барель для сайта, а этот компонент
 * читает контекст и годится только внутри постера.
 */
