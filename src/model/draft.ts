...
export function draftWillBeLost(title: string): string {
  return (
    `Без логина можно редактировать только один сезон. У вас уже сохранен «${title}» и это дейтсвие заменит его на новый. Войдите, чтобы иметь возможность редактировать и сохранять свои сезоны без ограничений.`
  )
}
