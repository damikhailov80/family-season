 ...
 *      уводим редиректом.
 *
 * Заодно в запрос уходят два заголовка. `x-lang-path` несёт путь без языка и
 * вместе с query — иначе лейауту некуда уводить: своего адреса серверный
 * компонент не знает, а вместе с адресом потерялась бы и примерка оформления.
...
export function proxy(request: NextRequest) {
  ...
  if (inPath) {
    ...
    const response = pass(
      request,
      auto ? 'auto' : 'url',
      // Вместе с query: по этому пути лейаут уводит на язык настройки, а в
      // query живёт примеренное оформление (`?p=`, `?i=`) и состояние списков
      // (`?tab=`, `?q=`, `?sort=`). Потерять их на редиректе значит потерять
      // ровно то, ради чего человек прислал ссылку.
