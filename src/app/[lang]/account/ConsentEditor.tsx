...
/**
 * Согласие на аналитику в кабинете.
 *
 * Отзыв обязан даваться так же легко, как согласие, — поэтому решение живёт не
 * только в баннере, который человек однажды закрыл и больше не увидит. Ссылка
 * «Куки» в подвале делает то же самое для невошедшего; здесь настройка стоит
 * рядом с языком, потому что это настройка аккаунта, а не разовый вопрос.
 *
 * Устроен близнецом `LanguageEditor` и по тем же причинам: успех уводит
 * редиректом с пометкой `?ok=1`, а неудача возвращается значением и
 * показывается тостом, не теряя выбранного.
 */
interface Failure {
